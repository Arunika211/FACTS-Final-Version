...
const AIAnalysisCard: React.FC<AIAnalysisCardProps> = ({ 
  ...
  const getStatusColor = () => {
    if (!analysis) return 'var(--info)';
    
    if (analysis.toLowerCase().includes('kritis') || 
        analysis.toLowerCase().includes('buruk') || 
        analysis.toLowerCase().includes('bahaya')) {
      return 'var(--danger)';
    } else if (analysis.toLowerCase().includes('waspada') || 
               analysis.toLowerCase().includes('perhatian') || 
  ...
  const getStatusIcon = () => {
    if (!analysis) return <FiInfo className="text-info" />;
          ...
          <div className="p-4 border-b flex justify-between items-center" 
            ...
            <h3 className="font-bold flex items-center gap-2 font-heading">
              ...
              <span style={{ color: getStatusColor() }}>
                {analysis.toLowerCase().includes('kritis') || analysis.toLowerCase().includes('bahaya') ? 'Status Kritis' : 
                analysis.toLowerCase().includes('waspada') || analysis.toLowerCase().includes('perhatian') ? 'Perlu Perhatian' : 'Kondisi Baik'}
              </span>
            </h3>
            <button 
